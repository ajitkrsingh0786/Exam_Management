...
<p style="margin-left:3%;">List of all students:</p>
	 ...
	 <table class="table table-stiped" >
			...
			%><tr>
				...
				<th> <a href=""> <%=student.getName()%> </a></th>
				<th><%=student.getAddress()%></th>
				<th><%=student.getContact()%></th>
				<th><%=student.getCourse()%></th>
			    <th>
			          <% Set set1=student.getSubject();
			             Iterator it1=set1.iterator();
			             while(it1.hasNext())
			             {
			            	 Subject sub1= (Subject)it1.next();
			            	 %>
			            	 <span><a href="AdminResultVerificationPage.jsp?id=<%=student.getS_id() %>&sub_id=<%=sub1.getSub_id() %>"><%=sub1.getSub_name() %> </a></span><br>
			            	 <%
